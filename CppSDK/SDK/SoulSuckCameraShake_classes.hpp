#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: SoulSuckCameraShake

#include "Basic.hpp"

#include "Engine_classes.hpp"


#pragma pack(push, 0x4)
namespace SDK
{

// BlueprintGeneratedClass SoulSuckCameraShake.SoulSuckCameraShake_C
// 0x0000 (0x0120 - 0x0120)
class USoulSuckCameraShake_C final : public UCameraShake
{
public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"SoulSuckCameraShake_C">();
	}
	static class USoulSuckCameraShake_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<USoulSuckCameraShake_C>();
	}
};
static_assert(alignof(USoulSuckCameraShake_C) == 0x000004, "Wrong alignment on USoulSuckCameraShake_C");
static_assert(sizeof(USoulSuckCameraShake_C) == 0x000120, "Wrong size on USoulSuckCameraShake_C");

}

#pragma pack(pop)
